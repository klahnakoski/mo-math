...
class GeneralTestEncryptionMode(unittest.TestCase):
    ...
    def run_cipher(self, cipher_mode, iv, ciphertext_list, plaintext_list):
        ...
        for k in range(len(ciphertext_list)):
            self.assertEquals(
                len([i for i, j in zip(ciphertext_list[k], cipher_mode.encrypt_block(plaintext_list[k])) if i == j]),
                16,
        ...
        for k in range(len(plaintext_list)):
            self.assertEquals(
                len([i for i, j in zip(plaintext_list[k], cipher_mode.decrypt_block(ciphertext_list[k])) if i == j]),
                16,
